...
import { applyAutoRag } from "./auto-rag.js";
...
/**
 * Minify structured content (JSON, CSS, JS/TS) deterministically.
 * For JS/TS: Uses AST-based minification via terser.
 ...
 * @returns {Promise<{ text: string, originalTokenEstimate: number, minifiedTokenEstimate: number, applied: boolean }>}
 */
export async function minifyStructuredContent(text, contentType = "auto") {
  ...
  try {
    ...
    if (detected === "js") {
      const result = await minify(text, { compress: false, mangle: false, format: { comments: false } });
      if (result.code) {
        return { text: result.code, originalTokenEstimate, minifiedTokenEstimate: estimateTokens(result.code), applied: true };
      }
    }
  } catch (err) {
...
export async function runTokenSavingPipeline(params, options = {}) {
  ...
  const meta = {
    ...
    autoRag: { applied: false, originalChars: 0, compressedChars: 0 },
  ...
  if (isStructuralMinifyEnabled() && workingParams.prompt) {
    const result = await minifyStructuredContent(workingParams.prompt);
    if (result.applied) {
  ...
  // --- AUTO-RAG: local context compression for massive prompts ---
  if (workingParams.prompt && process.env.AUTO_RAG_ENABLED !== "false") {
    const beforeChars = workingParams.prompt.length;
    try {
      workingParams = await applyAutoRag(workingParams);
      const afterChars = workingParams.prompt.length;
      if (afterChars < beforeChars) {
        meta.autoRag.applied = true;
        meta.autoRag.originalChars = beforeChars;
        meta.autoRag.compressedChars = afterChars;
        const saved = Math.round((beforeChars - afterChars) / 4); // rough token estimate
        meta.totalTokensSaved += saved;
        sessionStats.tier1Saved += saved;
      }
    } catch (err) {
      log(`token-saver: auto-rag failed (${err.message}), continuing without compression`);
    }
  }
