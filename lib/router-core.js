...
export const PROVIDER_REGISTRY = {
  ...
  cloudflare: { fn: callCloudflare, streamFn: streamCloudflare, models: CF_MODELS,        chatModels: CF_MODELS,         defaultModel: CF_DEFAULT,      supportsImages: false, supportsTools: false, supportsStream: false, supportsJsonMode: false },
...
export async function executeProviderChain({
  ...
  if (waitMs > 0 && waitMs !== Infinity) {
    return queueOrRun(order, keys, () => executeProviderChain({
      order, keys, model, params, tools, tool_choice, response_format,
      hasImage, session_id, sanitizeNote, verbose, onDelta, abortSignal,
    }));
  }
