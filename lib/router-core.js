...
import { executeEnsembleChain } from "./ensemble.js";
import { injectToolPolyfill, extractToolPolyfill } from "./tool-polyfill.js";
...
export const PROVIDER_REGISTRY = {
  gemini:     { fn: callGemini,     streamFn: streamGemini,     models: GEMINI_MODELS,    chatModels: GEMINI_MODELS,     defaultModel: GEMINI_DEFAULT,  supportsImages: true,  supportsTools: true,  supportsStream: true, supportsJsonMode: true  },
...
export async function executeProviderChain({
  ...
  if (model === "ensemble") {
    return await executeEnsembleChain(params, { order, keys, onDelta, abortSignal });
  }
  ...
  for (const providerName of order) {
    ...
    for (const chosenModel of modelCandidates) {
      ...
      try {
        let callParams = { ...params, maxTokens: effectiveMaxTokens, model: chosenModel, apiKey };
        ...
        if (globalStreamedTextSoFar) {
          const continuationPrompt = `\n\n[SYSTEM INSTRUCTION: You were generating a response but disconnected halfway. This is what you streamed so far. Continue your response EXACTLY from where you left off. Do NOT repeat anything from the text below.]\n${globalStreamedTextSoFar}`;
          callParams.systemPrompt = callParams.systemPrompt
            ? `${callParams.systemPrompt}${continuationPrompt}`
            : continuationPrompt;
        }
        if (tools?.length) {
          if (!entry.supportsTools) {
            callParams = injectToolPolyfill(callParams, tools);
          } else {
        ...
        if (useStreaming) {
          callParams.onDelta = (deltaText, deltaReason) => {
            globalStreamedTextSoFar += deltaText;
            onDelta(deltaText, deltaReason);
          };
        ...
        if (tools?.length && !entry.supportsTools && result.text) {
          const extractedTool = extractToolPolyfill(result.text);
          if (extractedTool) {
            result.toolCalls = [
              {
                id: `call_${Date.now()}`,
            ...
            result.text = ""; // clear text since it's a tool call
