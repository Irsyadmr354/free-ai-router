...
import { notifyProviderRecovered } from "./notifier.js";
...
export function cbRecordSuccess(provider) {
  ...
  const b = getBreaker(provider);
  if (b.state !== CLOSED) {
    const downtimeMs = b.openedAt ? Date.now() - b.openedAt : 0;
    log(`Circuit breaker: ${provider} → CLOSED (recovered)`);
    // Only fires when the breaker was actually OPEN/HALF_OPEN before this
    // success (i.e. a genuine recovery), not on every routine CLOSED success.
    notifyProviderRecovered(provider, downtimeMs).catch(() => {});
  }
  b.state = CLOSED;
  b.failures = [];
  b.openedAt = null;
